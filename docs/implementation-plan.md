...
## Phase 10: Strategy selection / allocation layer

目的:
- 戦略を増やし続けた場合の組み合わせ爆発を管理する
- 組み合わせごとの成績劣化を検出する
- 長期的に期待値の悪い組み合わせを停止候補として提示する

初期実装:
- recommendation only
- 実エントリー挙動には影響させない
- signals / opens / closes / baseline_* から集計する
- nが十分に溜まるまでは採用判断しない

禁止:
- 少数サンプルで自動最適化しない
- バックテストで良かった組み合わせを即live採用しない
- 実測ログを公開しない